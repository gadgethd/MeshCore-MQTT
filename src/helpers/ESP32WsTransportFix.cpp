...
//
// Ported from agessaman/MeshCore @ observer-firmware (MIT License), where the
// crash was decoded from a real Heltec V3 backtrace. See that fork's
// src/helpers/ESP32WsTransportFix.cpp for the original analysis.
//
...
// esp_mqtt_client_destroy(), i.e. on every MQTT slot reset in this fork
// (`mqtt reconnect N`) and at the start of the online-OTA path — and the free
// asserts:
//
...
// header). Only the first two members (path, buffer) are dereferenced here.
