...
static constexpr size_t LEGACY_NODE_PREFS_BYTES = 290;
static constexpr size_t NODE_PREFS_BYTES = 291;
...
static size_t packPrefs(uint8_t* buf, const NodePrefs* p) {
  ...
  WR(&p->rx_boosted_gain, sizeof(p->rx_boosted_gain));           // 290
  // 291
  #undef WR
...
static void unpackPrefs(const uint8_t* buf, NodePrefs* p) {
  ...
  RD(&p->rx_boosted_gain, sizeof(p->rx_boosted_gain));           // 290
  // 291
  #undef RD
}

// v1.14.x layout used byte 79 for rx_boosted_gain and ended at owner_info.
static void unpackLegacyPrefs(const uint8_t* buf, NodePrefs* p) {
  const uint8_t* r = buf;
  uint8_t pad[8];
...
bool CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {
  ...
  if (fsize < LEGACY_NODE_PREFS_BYTES) {
    Serial.printf("[prefs] loadPrefsInt: '%s' too small: %u < %u\n", filename, (unsigned)fsize, (unsigned)LEGACY_NODE_PREFS_BYTES);
    file.close();
    return false;
  }
  ...
  size_t expected_bytes = fsize >= NODE_PREFS_BYTES ? NODE_PREFS_BYTES : LEGACY_NODE_PREFS_BYTES;
  size_t bytes_read = file.read(buf, expected_bytes);
  file.close();

  if (bytes_read != expected_bytes) {
    Serial.printf("[prefs] loadPrefsInt: short read from '%s': %u / %u\n", filename, (unsigned)bytes_read, (unsigned)expected_bytes);
    return false;
  }

  if (expected_bytes == NODE_PREFS_BYTES) {
    unpackPrefs(buf, _prefs);
  } else {
    unpackLegacyPrefs(buf, _prefs);
  }
